      ...
      if (data.message == 'Berhasil Login') {
        if (data.data.id_role == 'RO-2003062') {
          ...
          const createFirebaseUser = async (email, password) => {
            try {
              const sukses = await configfirebase
                .auth()
          ...
          const loginFirebase = async (email, password) => {
            try {
              const responseberhasil = await configfirebase
                .auth()
          ...
          const getUserData = async (uid) => {
            try {
              const responsedatabase = await configfirebase
                .database()
                .ref(`users/ahli/${uid}`)
                .once('value')
          ...
          const getProfile = async () => {
            try {
              const profilekonsumen = await axios({
          ...
          const saveKonsumenData = async (uid, konsumenData) => {
            try {
              await configfirebase.database().ref(`users/konsumen/${uid}`).set(konsumenData)
            } catch (error) {
              console.log(error)
              throw error
            }
          ...
          if (datauser.uuid_firebase == null) {
            try {
              const profilekonsumen = await getProfile()
              const sukses = await createFirebaseUser(datauser.email, form.password)
              const datakonsumen = {
                id_konsumen: profilekonsumen.data.id_konsumen,
                nik: profilekonsumen.data.nik,
                nomor_hp: profilekonsumen.data.user.nomor_hp,
                email: profilekonsumen.data.user.email,
                nama: profilekonsumen.data.user.nama,
                uid: sukses
              }
              await updateFirebaseUid(profilekonsumen.data.user.id, sukses)
              await saveKonsumenData(sukses, datakonsumen)
              const responseberhasil = await loginFirebase(datauser.email, form.password)
              const userData = await getUserData(responseberhasil)
              storeData('user', userData)
              storeData('dataUser', {
        ...
        <ScrollView>
          <View style={styles.viewCard}>
            ...
            <View style={{ flexDirection: 'row' }}>
              <TouchableOpacity
                style={{ paddingLeft: 10, paddingTop: 10, flex: 1, alignItems: 'flex-start' }}
                onPress={() => {
              ...
              >
                <View style={{ alignItems: 'flex-end', paddingRight: 10 }}>
                  <Text style={{ color: 'red', fontSize: 14 }}>Lupa Password ?</Text>
                </View>
              </TouchableOpacity>
            </View>
          </View>
